...
public interface AdminMatchingApi {
	...
	ResponseEntity<ResponseDto<List<MatchingResponseDto>>> allMatching(HttpServletRequest request, @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size);
	...
	@ApiResponses({@ApiResponse(responseCode = "200", description = "조회 성공"),
		...
		@ApiResponse(responseCode = "403", description = "권한 없음"),
		@ApiResponse(responseCode = "500", description = "데이터베이스 오류")})
	ResponseEntity<ResponseDto<List<MatchingResponseDto>>> statusMatching(@RequestParam Status status, HttpServletRequest request, @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size);
